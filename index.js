...
ControlChromecastPlatform.prototype.scanAccesories = function () {
  ...
  let addChromecast = function(device){
    ...
    if(device && device.txtRecord && ['Chromecast','Lenovo CD-4N341Y', 'Google Cast Group', 'X04G'].indexOf(device.txtRecord.md) !== -1){
...
ControlChromecastPlatform.prototype.addAccessory = function (device) {
    ...
    accessory.addService(Service.Television, accessory.context.name);

    this.accessories[accessory.UUID] = new ChromecastAccessory(this.log, accessory, device);

    //this.api.registerPlatformAccessories('homebridge-chromecast-television', 'ControlChromecast', [accessory]);
    this.api.publishExternalAccessories('homebridge-chromecast-television', [accessory]);
};
...
ControlChromecastPlatform.prototype.removeAccessory = function (accessory) {
  ...
  //this.api.unregisterPlatformAccessories('homebridge-chromecast-television', 'ControlChromecast', [accessory]);
};
...
ControlChromecastPlatform.prototype.configurationRequestHandler = function (context, request, callback) {  // eslint-disable-line
  ...
  switch (context.onScreen) {
    ...
    case 'DoModify':
      ...
      var service = context.accessory.getService(Service.Television);
...
function ChromecastAccessory(log, accessory, device) {
  ...
  let service = this.accessory.getService(Service.Television);
  ...
  if (service.testCharacteristic(Characteristic.On) === false) {
    service.addCharacteristic(Characteristic.On)
  }
...
ChromecastAccessory.prototype.addEventHandler = function (service, characteristic) {
  ...
  switch (characteristic) {
    ...
    case Characteristic.SleepDiscoveryMode:
      service
        .getCharacteristic(Characteristic.SleepDiscoveryMode)
        .on('get', 1)
        .on('set', 1);
      break;
    case Characteristic.RemoteKey:
      service
